...
export class LfvAirspaceService {
  ...
  private isWithinRadius(lat: number, lng: number, restriction: AirspaceRestriction, radiusM: number): boolean {
    ...
    if (geom.type === 'Polygon' && geom.coordinates?.[0]) {
      ...
      const centroidLat = coords.reduce((s: number, c: GeoJSON.Position) => s + c[1], 0) / coords.length
      const centroidLng = coords.reduce((s: number, c: GeoJSON.Position) => s + c[0], 0) / coords.length
