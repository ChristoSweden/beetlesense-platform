  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[var(--text)] mb-2">Drone Verification Workflow</h1>
          <p className="text-gray-600">Satellite → Drone → AI verification pipeline. Confirm anomalies detected by Sentinel-2 with high-resolution drone imagery.</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          <div className="bg-[var(--bg2)] rounded-lg p-4 shadow-sm border"><div className="text-2xl font-bold">{missions.length}</div><div className="text-sm text-gray-500">Total Missions</div></div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {['all', 'pending', 'scheduled', 'in-flight', 'processing', 'verified', 'rejected'].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} className={`px-3 py-1.5 rounded-full text-xs font-medium ${filterStatus === s ? 'bg-[var(--green)] text-white' : 'bg-[var(--bg2)] text-gray-600 border hover:bg-[var(--bg)]'}`}>{s === 'all' ? 'All' : STATUS_CONFIG[s]?.label || s}</button>
          ))}
        </div>
              ...
              <button key={mission.id} onClick={() => setSelectedMission(mission)} className={`w-full text-left ${cfg.bg} rounded-xl p-5 border-2 border-transparent hover:border-blue-400 transition-all ${selectedMission?.id === mission.id ? 'ring-2 ring-blue-500' : ''}`}>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-[var(--text)]">{mission.standName}</h3>
