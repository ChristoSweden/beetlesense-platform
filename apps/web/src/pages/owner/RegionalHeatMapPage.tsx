...
const RISK_COLORS: Record<string, { bg: string; border: string; text: string; badge: string }> = {
  critical: { bg: 'bg-red-100', border: 'border-red-500', text: 'text-red-700', badge: 'bg-red-500 text-white' },
  high: { bg: 'bg-orange-100', border: 'border-orange-500', text: 'text-orange-700', badge: 'bg-orange-500 text-white' },
  moderate: { bg: 'bg-yellow-100', border: 'border-yellow-500', text: 'text-yellow-700', badge: 'bg-yellow-500 text-white' },
  low: { bg: 'bg-green-100', border: 'border-green-500', text: 'text-green-700', badge: 'bg-green-500 text-white' },
};
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[var(--text)] mb-2">Regional Susceptibility Heat Map</h1>
          <p className="text-gray-600">County-level bark beetle risk based on Skogsstyrelsen data, climate models, and forest composition.</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-red-50 rounded-lg p-4 border border-red-200"><div className="text-2xl font-bold text-red-600">{criticalCount}</div><div className="text-sm text-red-700">Critical Counties</div></div>
          <div className="bg-orange-50 rounded-lg p-4 border border-orange-200"><div className="text-2xl font-bold text-orange-600">{highCount}</div><div className="text-sm text-orange-700">High Risk Counties</div></div>
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200"><div className="text-2xl font-bold text-blue-600">{SWEDISH_COUNTIES.length}</div><div className="text-sm text-blue-700">Counties Monitored</div></div>
          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200"><div className="text-2xl font-bold text-purple-600">6h</div><div className="text-sm text-purple-700">Update Frequency</div></div>
        </div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {['all', 'critical', 'high', 'moderate', 'low'].map(level => (
            <button key={level} onClick={() => setFilterLevel(level)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filterLevel === level ? 'bg-[var(--green)] text-white' : 'bg-[var(--bg2)] text-gray-700 border border-gray-300 hover:bg-[var(--bg)]'}`}>
            ...
            return (
              <button key={county.id} onClick={() => setSelectedCounty(county)}
                ...
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-lg font-bold text-[var(--text)]">{county.name}</h3>
                  <span className={`${colors.badge} px-3 py-1 rounded-full text-xs font-bold uppercase`}>{county.riskLevel}</span>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-gray-600">Risk Score</span><span className={`font-bold ${colors.text}`}>{county.riskScore}/100</span></div>
                  ...
                  <div className="flex justify-between text-sm"><span className="text-gray-600">Spruce Coverage</span><span className="font-medium text-[var(--text)]">{county.sprucePercentage}%</span></div>
                  <div className="flex justify-between text-sm"><span className="text-gray-600">Degree-Days</span><span className="font-medium text-[var(--text)]">{county.currentDegreeDays}/{county.threshold}°d</span></div>
                </div>
              </button>
            );
          })}
        ...
        {/* Detail Panel */}
        {selectedCounty && (
          <div className="bg-[var(--bg2)] rounded-xl shadow-lg p-6 border border-[var(--border)]">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-[var(--text)]">{selectedCounty.name} — Detailed Analysis</h2>
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <h3 className="font-semibold text-blue-900 mb-1">Recommended Action</h3>
