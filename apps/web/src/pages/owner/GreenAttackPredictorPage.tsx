...
export default function GreenAttackPredictorPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Green Attack Window Predictor</h1>
      <p className="text-gray-600 mb-6">Predicts the 7\u201314 day intervention window before bark beetle damage becomes irreversible. Based on degree-day accumulation (&gt;5\u00B0C) and NDVI spectral change from Sentinel-2.</p>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {DEMO_STANDS.map(s => (
          <button key={s.id} onClick={() => setSelected(s)}
            className={`p-4 rounded-xl border-2 text-left transition-all ${selected.id === s.id ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-gray-300'}`}>
            <div className="font-semibold text-gray-900">{s.name}</div>
            <div className="text-sm text-gray-500">{s.county}</div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        ...
        <p className="text-sm text-gray-600">{progress}% toward one full Ips typographus generation cycle. Threshold: 334 degree-days above 5\u00B0C.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl shadow p-4 text-center">
          <div className="text-3xl font-bold text-green-700">{selected.confidence}%</div>
          <div className="text-sm text-gray-500">Prediction confidence</div>
        </div>
        <div className="bg-white rounded-xl shadow p-4 text-center">
          <div className="text-lg font-bold text-gray-900">{selected.windowStart} \u2192 {selected.windowEnd}</div>
          <div className="text-sm text-gray-500">Intervention window</div>
        </div>
        <div className="bg-white rounded-xl shadow p-4 text-center">
          <div className="text-3xl font-bold text-amber-600">{selected.ndviDelta}</div>
          <div className="text-sm text-gray-500">NDVI change (Sentinel-2)</div>
        </div>
      </div>
    </div>
  );
